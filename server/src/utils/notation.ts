...
export interface NoteToken {
  raw: string;
  pitch: string;          // 纯音高数字 "1"-"7"，和弦时为最高音
  ...
  chordNotes?: ChordSubNote[]; // 和弦内各音符
}

/** 和弦子音符（单个音高 + 八度 + 变音） */
export interface ChordSubNote {
  pitch: string;          // "1"-"7"
  octaveDots: number;     // 高音/低音点
  accidental?: string;    // "#", "b"
}
...
export function parseJianpu(content: string | null): ParsedScore {
  ...
  for (const line of contentLines) {
    ...
    for (let m = 0; m < measureStrs.length; m++) {
      ...
      // 和弦感知分词：保护 [...] 内的空格
      const noteStrs = tokenizeMeasureWithChords(raw);
...
export function parseNoteToken(raw: string, beatUnit?: number): NoteToken {
  // 和弦格式 [1 3 5]
  if (raw.startsWith('[')) {
    return parseChordToken(raw, beatUnit);
  }
  return parseSingleNoteToken(raw, beatUnit);
}

/** 和弦感知分词：遇到 [...] 保持为一个 token */
function tokenizeMeasureWithChords(raw: string): string[] {
  const escaped = raw.replace(/\[([^\]]+)\]/g, (m) => m.replace(/ /g, '\x00'));
  const tokens = escaped.split(/\s+/).filter(Boolean);
  return tokens.map((t) => t.replace(/\x00/g, ' '));
}

/** 解析和弦音符标记，格式：[1 3 5]- 或 [˙1 3 .5] 等 */
export function parseChordToken(raw: string, beatUnit?: number): NoteToken {
  const defaultDuration = beatUnit === 8 ? 8 : 4;
  const m = raw.match(/^\[([^\]]*)\](.*)$/);
  if (!m) return parseSingleNoteToken(raw, beatUnit);
  const inner = m[1].trim();
  const suffix = m[2];

  // 解析内部分音
  const parts = inner.split(/\s+/).filter(Boolean);
  const chordNotes: ChordSubNote[] = parts.map(p => parseChordSubNote(p));
  if (!chordNotes.length) return parseSingleNoteToken('0', beatUnit);

  // 从后缀解析时值/连线/演奏法
  const token: NoteToken = {
    raw,
    pitch: chordNotes.map(n => n.pitch).join(''),
    octaveDots: 0,
    duration: defaultDuration,
    isChord: true, chordNotes,
  ...
  let w = suffix;
  if (w.startsWith('~')) { token.hasTieEnd = true; w = w.slice(1); }
  if (w.endsWith('~')) { token.hasTie = true; w = w.slice(0, -1); }
  if (w.endsWith('.')) { token.isStaccato = true; w = w.slice(0, -1); }
  if (w.includes('>')) { token.isAccent = true; w = w.replace('>', ''); }

  const underMatch = w.match(/_+$/);
  if (underMatch) {
  ...
  const hyphenMatch = w.match(/-+$/);
  if (hyphenMatch && !underMatch) {
    const hc = hyphenMatch[0].length;
    if (hc >= 3) token.duration = 1;
    else if (hc === 2) { token.duration = 2; token.isDot = true; }
    else if (hc === 1) token.duration = 2;
    w = w.slice(0, -hc);
  }
  return token;
}
...
/** 解析和弦子音符（单个音高 + 八度 + 变音） */
export function parseChordSubNote(text: string): ChordSubNote {
  let work = text.trim();
  let acc: string | undefined;
  let dots = 0;

  if (work.startsWith('#')) { acc = '#'; work = work.slice(1); }
  else if (work.startsWith('b')) { acc = 'b'; work = work.slice(1); }

  // 高音点 ˙
  const hi = work.match(/^˙+/);
  if (hi) { dots = hi[0].length; work = work.slice(hi[0].length); }
  // 低音点 .
  const lo = work.match(/^\.+/);
  if (lo) { dots = -lo[0].length; work = work.slice(lo[0].length); }

  const pitch = work.replace(/[^\d]/g, '') || '1';
  return { pitch, octaveDots: dots, accidental: acc };
}

/** 解析单个普通音符 */
function parseSingleNoteToken(raw: string, beatUnit?: number): NoteToken {
